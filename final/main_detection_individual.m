...
[trainPaths, trainLabels, testPaths, testLabels,valPaths,valLabels] = ...
    preprocess_detection(datasetPath,patients(p),numChannels,downsample);
...
inputSize = [size(trainPaths,1), size(trainPaths,2)];
prog = "Starting Training"

%net = buildNetwork_detection(dsTrain,dsVal,inputSize,filterSize,numFilters,...
%    maxPool);
net = buildNetwork_detection(trainPaths,dsVal,inputSize,filterSize,numFilters,...
    maxPool,trainLabels);
